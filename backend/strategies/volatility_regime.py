...
class VolatilityRegimeStrategy(BaseStrategy):
    ...
    async def analyze(self, df: pd.DataFrame, ticker: Ticker) -> Signal:
        ...
        vol_sma = row.get("volume_sma_20", row.get("Volume_SMA_20", 0))
        ...
        indicators = {
            ...
            else ("high" if atr_percentile > self._high_pct else "normal"),
            "rsi": round(float(rsi), 2),
            "band_width_pct": round(band_width * 100, 2),
            "volume_ratio": round(float(volume) / float(vol_sma), 2)
            if vol_sma and float(vol_sma) > 0
            else 0,
        }
        ...
        if atr_percentile < self._low_pct:
            vol_confirmed = (
                float(vol_sma) > 0
                and float(volume) > float(vol_sma) * self._vol_confirm_mult
            )
        ...
        if atr_percentile > self._high_pct:
            ...
            if float(rsi) < 30 and price <= float(bb_lower) * 1.01:
                ...
                rsi_rising = (
                    float(rsi) > float(prev_rsi) if not pd.isna(prev_rsi) else False
                )
