...
| Main Spot History 기간 토글 (2026-04-11) | `TradingAccountOverview.tsx`의 `Main Spot History` 카드에 `전체 / 30d / 7d` 토글을 추가했다. `전체`는 `binance_spot` ledger 누적값(`PortfolioSummary`)을, `30d`/`7d`는 최근 실현손익/거래수(`TradeSummary`)를 보여줘 누적 계정 손익과 최근 흐름을 빠르게 비교할 수 있다. |
| Live 전략 기간 토글 (2026-04-11) | `TradingAccountOverview.tsx`의 `Donchian Spot`, `Donchian Futures`, `Pairs Trading` 카드에 공용 `today / 7d / 30d` 토글을 추가했다. 선택 기간에 따라 거래수와 실현손익을 같은 기준으로 비교하고, 누적 손익과 현재 포지션/상태는 그대로 유지해 live 전략 간 성과 비교를 쉽게 만들었다. |
| Live 전략 평가 상태 가시화 (2026-04-11) | `donchian_daily_engine.py`, `donchian_futures_bi_engine.py`, `pairs_trading_live_engine.py`의 status에 `last_evaluated_at`, `next_evaluation_at`, `recent_idle_reason`를 추가했다. Overview 카드에는 최근 평가 시각, 다음 평가 예정, 최근 대기 사유를 노출해 “엔진은 돌지만 왜 아직 체결이 없는지”를 바로 읽을 수 있게 했다. 정지 엔진은 `next_evaluation_at=null`로 내려 실제 예약과 혼동되지 않도록 정리했고, 평가 후엔 `Donchian`의 가장 가까운 돌파 갭과 `Pairs`의 현재 `z-score` 대비 진입 gap까지 표시한다. |
