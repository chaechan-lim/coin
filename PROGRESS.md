...
| BalanceGuard 선물 잔고 수정 + 자동 재동기화 (COIN-19) | `_fetch_exchange_balance()` 선물 잔고 계산을 `USDT.free` → `walletBalance - unrealizedPnL - totalMargin`으로 수정 (PM 초기화 로직과 동일). 자동 재동기화: N회(기본 5) 연속 critical → `initialize_cash_from_exchange()` 재호출 + 자동 resume. `BalanceCheckResult.resynced` 필드 추가. `FuturesV2Config.balance_auto_resync_count` 설정 추가. |
