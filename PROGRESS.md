...
| 선물 파생 데이터 파이프라인 (COIN-79) | OI, Mark Price, 프리미엄, Top Trader 롱/숏 비율 수집 파이프라인 추가. **데이터 모델**: `OpenInterest`, `MarkPriceInfo`, `LongShortRatio` 3개 dataclass. **어댑터**: `BinanceUSDMAdapter`에 `fetch_open_interest()` (CCXT), `fetch_open_interest_history()` (fapiPublic), `fetch_mark_price()` (fapiPublicGetPremiumIndex), `fetch_long_short_ratio()` (fapiPublicGetTopLongShortAccountRatio) 4개 메서드. `ExchangeAdapter` ABC에 4개 optional 메서드. **DerivativesDataService**: 인메모리 캐시 + 시계열 deque + TTL staleness + 5분 주기 수집 루프 + graceful degradation. **RegimeDetector**: `derivatives_data` 선택적 주입, `detect()`에서 `derivatives_snapshot` dict 생성 (oi_value, premium_pct, long_short_ratio, funding_rate, is_stale). `RegimeState`에 `derivatives_snapshot` 필드. **FuturesEngineV2**: `DerivativesDataService` 생성→RegimeDetector 주입→start/stop 연동. 테스트 48개 추가(2164 total). |
